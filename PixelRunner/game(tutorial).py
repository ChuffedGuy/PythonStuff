...
def display_score():
    ...
    score_surf = test_font.render(f'{int(current_time/1000)}', False, (64, 64, 64))
...
def background():
    sky_rect_list = [sky_rect, sky_rect_2]
    ground_rect_list = [ground_rect, ground_rect_2]

    for rect in sky_rect_list:
        rect.x -= 1
        if rect.right <= 0:
            rect.left = 800
...
pygame.display.set_caption('Pooper')
...
ground_rect_2 = ground_surface.get_rect(topleft = (800, 300))
    ...
    for event in pygame.event.get():
        ...
        if game_active:
            if event.type == obstacle_timer:
                obstacle_group.add(Obstacle(choice(['fly', 'snail', 'snail', 'snail'])))    # Enemy Spawner
        else:
